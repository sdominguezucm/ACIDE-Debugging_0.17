...
/**
 * ACIDE - A Configurable IDE close database connection menu item action.
 * 
 * @version 0.11
 * @see ActionListener
 */
public class AcideCloseDatabaseConnectionMenuItemAction implements ActionListener{
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		String result = des.closeConnection(tree.getLastPathComponent().toString());
		
		boolean success = true;

		success = result.contains("$success");
		if(success){
			panel.getTree().removeSelectionPath(tree);
