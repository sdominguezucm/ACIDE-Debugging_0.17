...
/**
 * ACIDE - A Configurable IDE constraints window NN panel.
 * 
 * @version 0.16
 * @see JPanel
 */
public class AcideNotNullPanel extends JPanel{
	...
	private HashMap<String, Boolean> _notNullKeys;
	...
	public AcideNotNullPanel(String database, String name){
		...
		_notNullKeys = new HashMap<String, Boolean>();
	...
	/**
	 * Initializes the table with the possible existing not null constraints
	 */
	private void initializeAttributes() {
		...
		clearNames(des);
		 ...
		 for(int i=0;i<_names.size();i++){
			 _notNullKeys.put(_names.get(i), Boolean.FALSE);
		 }
	...
	/**
	 * Assigns a TRUE value to each attribute that is a not null constraint
	 * @param database
	 */
	private void setNotNullKeys(AcideDatabaseManager des) {
		
		LinkedList<String> keys = des.getNullables(_database, _tableName);
			...
			for(int i=0;i<keys.size();i++){
				...
				if (aux.contains(",")){
					...
					for(int j=0;j<aux2.length;j++)
						_notNullKeys.put(aux2[j], Boolean.TRUE);
						
				} 
				else _notNullKeys.put(aux, Boolean.TRUE);	
			}
		}
	}
	...
	/**
	 * Builds a table with not null restrictions
	 */
	...
	public void saveChanges(){
		...
		if (!restriction.equals(_oldRestriction)){
			...
			String command = ":-nn("+ _tableName + ",[" + restriction + "])";
			String result = "";
	...
	/**
	 * Deletes the existing not null restriction
	 * @return result
	 */
	private String dropNulls() {
		AcideDatabaseManager des = AcideDatabaseManager.getInstance();
		LinkedList<String> nnKeys = des.getNullables(_database, _tableName);
		String result = "";
		
		if (!nnKeys.isEmpty()){
			for(int i=0;i<nnKeys.size();i++){
				String command = ":-nn("+ _tableName + "," + nnKeys.get(i) + ")";
	...
	private class MyTableModel extends AbstractTableModel{
		...
		private String[] headers = {AcideLanguageManager.getInstance().getLabels().getString("s2299")
				...
				,AcideLanguageManager.getInstance().getLabels().getString("s2109")};
		...
		public void build(){
			...
			for(int i=0; i<_names.size(); i++){
				...
				data[i][2] = _notNullKeys.get(_names.get(i));
			}
