...
public class TreeLayout implements GraphLayout {
	...
	/**
	 * ACIDE - A Configurable IDE tree  layout margin
	...
	/*
	 * (non-Javadoc)
	 * @see acide.gui.graphLayout.GraphLayout#calculateNodesPosition(acide.gui.graphUtils.DirectedWeightedGraph, int, int, int)
	 */
	@Override
	...
	private int setNodePosition(Node node, DirectedWeightedGraph graph,
		...
		if(node.isPosicionado())
			...
			for(DirectedWeightedLink l : links){
				if(_mode.equals(INVERSE_MODE) && l.getDestiny().equals(node) && 
						!l.getOrigin().equals(node) &&
						!l.getOrigin().isPosicionado())
					children.add(l.getOrigin());
				else
					if(_mode.equals(DIRECT_MODE) && l.getOrigin().equals(node) && 
