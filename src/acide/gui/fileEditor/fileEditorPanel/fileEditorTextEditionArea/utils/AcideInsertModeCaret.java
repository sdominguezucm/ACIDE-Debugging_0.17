...
/**
 * ACIDE - A Configurable IDE insert mode caret.
 * 
 * A wider caret to highlight in a more intense way the caret position in the
 * edition text edition areas when the insert mode is selected.
 * 
 * @version 0.11
 * @see DefaultCaret
 */
public class AcideInsertModeCaret extends DefaultCaret {

	/**
	 * ACIDE - A Configurable IDE insert mode caret class serial version UID.
	 */
	...
	/**
	 * Creates a new instance of ACIDE - A Configurable IDE insert mode caret.
	 */
	public AcideInsertModeCaret() {
	...
	public void paint(Graphics g) {
		...
		if (isVisible()) {
			try {
				...
				Rectangle r = component.getUI().modelToView(component,
						getDot(), _dotBias);
	...
	/**
	 * Rectangle.contains returns false if passed a rectangle with a w or h ==
	 * 0, this won't (assuming X,Y are contained with this rectangle).
	 * 
	 * @param X
	 *            x coordinate.
	 * @param Y
