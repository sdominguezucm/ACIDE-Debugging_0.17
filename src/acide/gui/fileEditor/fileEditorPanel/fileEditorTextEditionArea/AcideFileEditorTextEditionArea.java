...
import acide.gui.fileEditor.fileEditorPanel.fileEditorTextEditionArea.utils.AcideUndoTabAction;
...
public class AcideFileEditorTextEditionArea {
	...
	private void setTextComponentKeyBindings() {
		...
		//Adds the SHIF+TAB action to the the text component
		_textComponent.getInputMap().put(KeyStroke.getKeyStroke("shift TAB"), "Undo TAB");
		
		// Puts the undo action with the SHIFT+TAB
		_textComponent.getActionMap().put(
						"Undo TAB", new AcideUndoTabAction());
	}
	...
	public void setLookAndFeel() {
		...
		//Sets the tab key size from the file editor configuration
		_textComponent.setTabSize(AcideWorkbenchConfiguration.getInstance()
				.getFileEditorConfiguration().getTabSize(), AcideWorkbenchConfiguration.getInstance()
	...
	public void setTextPaneListeners() {
		...
		// Adds the ACIDE - A Configurable IDE insert key listener
		_textComponent.addKeyListener(new AcideInsertKeyListener());
