...
/**
 * ACIDE - A Configurable IDE undo action.
 * 
 * @version 0.11
 * @see AbstractAction
 */
public class AcideUndoAction extends AbstractAction {

	/**
	 * ACIDE - A Configurable IDE undo action serial version UID.
	 */
	private static final long serialVersionUID = 1L;

	/**
	 * Creates a new ACIDE - A Configurable IDE undo action.
	 * 
	 * @param name
	 *            action name.
	 */
	public AcideUndoAction(String name) {
		super(name);
	}
		...
		try {
			...
			if (selectedFileEditorPanelIndex != -1) {
				...
				if (AcideUndoManager.getInstance().canUndo())
					...
					// Undo
					AcideUndoManager.getInstance().undo();
					//AcideUndoManager.getInstance().undo();

				// Updates the undo menu item option
				AcideMainWindow.getInstance().getMenu().getEditMenu()
						.getUndoMenuItem()
						.setEnabled(AcideUndoManager.getInstance().canUndo());
			}
