...
public class AcideSearchConsoleMenuItemListener implements ActionListener {
	...
	public static void action(ActionEvent actionEvent){
		// Puts the focus in the console panel
				...
				if (AcideConsolePanelSearchWindow.getInstance().isShowing())
					...
					if (selectedText != null){
						AcideConsolePanelSearchWindow.getInstance().getSearchTextField().setText(selectedText);
					}
					else {
						AcideConsolePanelSearchWindow.getInstance().getSearchTextField().setText("");
					}
	}
}
