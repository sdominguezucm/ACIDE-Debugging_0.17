...
/**
 * ACIDE - A Configurable IDE odbc panel menu item listener.
 * 
 * @version 0.16
 * @see ActionListener
 */
public class AcideOdbcPanelMenuItemListener implements ActionListener{
	/*
	 * (non-Javadoc)
	 * 
	 * @see
	...
	public static void action(ActionEvent actionEvent){
		...
		AcideMainWindow.getInstance().setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
		
//		AcideMainWindow.getInstance().getMenu().getConfigurationMenu().getDatabasePanelMenu()
//			.setShowDetailsEnabled(false);
	}
}
