 ...
 * @version 0.16
 */
public class AcideDebugPanelNodeSizeMenuItemListener implements ActionListener {
	/**
	...
	public static void action(ActionEvent actionEvent){
		...
		int acutalSize = 0;
		if(AcideMainWindow.getInstance().getDebugPanel().getTraceDatalogPanel().getCanvas() != null)
			acutalSize = AcideMainWindow.getInstance().getDebugPanel().getTraceDatalogPanel().getCanvas().getNodeSize();
		...
		if(option == JOptionPane.OK_OPTION && AcideGraphCanvas.getInstance() != null){
			AcideMainWindow.getInstance().getDebugPanel().getTraceDatalogPanel().getCanvas().setNodeSize((Integer)spinner.getValue());
