...
/**
 * ACIDE - A Configurable IDE selected node color menu item listener.
 * 
 * @see ActionListener
 * @version 0.16
 */
public class AcideDebugPanelSelectedNodeColorItemMenuItemListener implements
		ActionListener {
	...
	public static void action(ActionEvent actionEvent){
		//shows a dialog to choose, the color, set the chosen color to the node an repaint the canvas
		Color c =JColorChooser.showDialog(AcideMainWindow.getInstance(), "Color", AcideGraphCanvas.getInstance().getNodeColor());
		if(c!=null){
			AcideMainWindow.getInstance().getDebugPanel().getTraceDatalogPanel().getCanvas().setSelectedNodeColor(c);
