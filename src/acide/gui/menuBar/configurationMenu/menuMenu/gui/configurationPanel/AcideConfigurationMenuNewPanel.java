 ...
 * @see JPanel
 */
public class AcideConfigurationMenuNewPanel extends JPanel{
	/**
	 * ACIDE - A Configurable IDE configuration menu panel serial version UID.
	 */
	private static final long serialVersionUID = 1L;
	/**
	 * ACIDE - A Configurable IDE configuration menu panel configuration.
	 */
	...
	/**
	 * ACIDE - A Configurable IDE configuration menu panel file menu instance.
	 */
	private AcideConfigurationMenu _configurationMenu = AcideMainWindow.getInstance().getMenu()
			.getConfigurationMenu();
	/**
	 * ACIDE - A Configurable IDE  menu panel components.
	 */
	private HashMap<String, JCheckBox> _components = new HashMap<String, JCheckBox>();
	/**
	 * ACIDE - A Configurable IDE  menu panel are there changes.
	 */
	private static boolean _areThereChanges;
	/**
	 * ACIDE - A Configurable IDE  menu panel selected row.
	 */
	private int _selectedRow;
	
	
	/**
	 * Returns the ACIDE - A Configurable IDE configuration menu new panel
	 * configuration.
	 * 
	 * @return the ACIDE - A Configurable IDE configuration menu new panel
	 * configuration.
	 */
	...
	/**
	 * Sets the ACIDE - A Configurable IDE configuration menu new panel configuration.
	 * @param configuration
	 ...
	 */
	public AcideConfigurationMenuNewPanel() {
	...
	/**
	 * Sets the ACIDE - A Configurable IDE configuration menu new panel 
	 * data configuration.
	 */
	public void setDataFromConfiguration(){
		// Updates the configuration panel configuration panel table model
		// with the data
		((AcideConfigurationMenuNewPanelTableModel) getTable().getModel())
				.setItems(_configuration.getItemsManager().getAllObjectsWithNumber());
	}
	
	/**
	 * Sets the listeners for the ACIDE - A Configurable IDE configuration panel
	...
	/**
	 * Adds the components to the ACIDE - A Configurable IDE configuration panel.
	 */
	...
	public void buildTable() {
		...
		_table = new JTable(new AcideConfigurationMenuNewPanelTableModel(
				this));
	...
	private JPopupMenu createContextMenu(final int rowIndex,
		...
		AcideMenuObjectConfiguration ob = ((AcideConfigurationMenuNewPanelTableModel) getTable().getModel())
	...
	/**
	 * Builds and adds the components to the ACIDE - A Configurable IDE configuration
	 * menu panel.
	 */
	public void initComponents() {

		for (int index = 0; index < _configurationMenu.getItemCount(); index++) {

			JMenuItem menuItem = null;
			try {

				// Gets the menu item from the configuration menu
				menuItem = _configurationMenu.getItem(index);
				...
				// If the configuration JMenu has a menu instead of a JMenuItem

				// Gets the menu from the configuration menu
				JMenu menu = (JMenu) _configurationMenu.getMenuComponent(index);
	...
	public AcideMenuObjectConfiguration getSelectedItem() {
		if (getTable().getSelectedRow() > -1){
			return ((AcideConfigurationMenuNewPanelTableModel) getTable().getModel())
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (name != null){
				...
				String path;
					...
					((AcideConfigurationMenuNewPanelTableModel) getTable().getModel()).fireTableDataChanged();
					} else {
						path = "Root/";
						submenu = menu;
						setAreThereChanges(menu.AddSubmenu(submenu, name, path));
