...
/**
 * ACIDE - A Configurable IDE project menu new panel.
 * 
 * @version 0.11
 * @see JPanel
 */
public class AcideProjectMenuNewPanel extends JPanel  {
	/**
	 * ACIDE - A Configurable IDE project menu panel serial version UID.
	 */
	private static final long serialVersionUID = 1L;
	/**
	 * ACIDE - A Configurable IDE configuration project menu panel configuration.
	 */
	...
	/**
	 * ACIDE - A Configurable IDE project menu panel file menu instance.
	 */
	...
	/**
	 * ACIDE - A Configurable IDE project menu panel are there changes.
	 */
	private static boolean _areThereChanges;
	/**
	 * ACIDE - A Configurable IDE project menu panel selected row.
	 */
	...
	/**
	 * Sets the ACIDE - A Configurable IDE project menu new panel configuration.
	 * @param configuration
	 ...
	 */
	public AcideProjectMenuNewPanel() {
	...
	/**
	 * Sets the ACIDE - A Configurable IDE project menu new panel 
	 * data configuration.
	 */
	public void setDataFromConfiguration(){
		// Updates the project panel configuration panel table model
		// with the data
		((AcideProjectMenuNewPanelTableModel) getTable().getModel())
				.setItems(_configuration.getItemsManager().getAllObjectsWithNumber());
	}
	
	/**
	 * Sets the listeners for the ACIDE - A Configurable IDE project panel
	...
	/**
	 * Adds the components to the ACIDE - A Configurable IDE project panel.
	 */
	...
	public void buildTable() {
		...
		_table = new JTable(new AcideProjectMenuNewPanelTableModel(this));
	...
	private JPopupMenu createContextMenu(final int rowIndex,
		...
		AcideMenuObjectConfiguration ob = ((AcideProjectMenuNewPanelTableModel) getTable().getModel())
	...
	public AcideMenuObjectConfiguration getSelectedItem() {
		if (getTable().getSelectedRow() > -1){
			return ((AcideProjectMenuNewPanelTableModel) getTable().getModel())
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (name != null){
				...
				String path;
					...
					((AcideProjectMenuNewPanelTableModel) getTable().getModel()).fireTableDataChanged();
					} else {
						path = "Root/";
						submenu = menu;
						setAreThereChanges(menu.AddSubmenu(submenu, name, path));
