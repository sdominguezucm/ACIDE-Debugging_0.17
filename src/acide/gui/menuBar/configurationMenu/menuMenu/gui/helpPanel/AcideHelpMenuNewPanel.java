...
/**
 * ACIDE - A Configurable IDE help menu new panel.
 * 
 * @version 0.11
 * @see JPanel
 */
public class AcideHelpMenuNewPanel  extends JPanel {
	/**
	 * ACIDE - A Configurable IDE help menu panel serial version UID.
	 */
	private static final long serialVersionUID = 1L;
	/**
	 * ACIDE - A Configurable IDE help menu panel configuration.
	 */
	private AcideMenuSubmenuConfiguration _configuration;
	/**
	 * ACIDE - A Configurable IDE menu new configuration window table of help menu commands.
	 */
	...
	private AcideHelpMenu _helpMenu = AcideMainWindow.getInstance().getMenu()
			.getHelpMenu();
	/**
	 * ACIDE - A Configurable IDE help menu panel components.
	 */
	private HashMap<String, JCheckBox> _components = new HashMap<String, JCheckBox>();
	/**
	 * ACIDE - A Configurable IDE  help panel are there changes.
	 */
	private static boolean _areThereChanges;
	/**
	 * ACIDE - A Configurable IDE  help panel selected row.
	 */
	private int _selectedRow;
	
	
	/**
	 * Returns the ACIDE - A Configurable IDE help menu new panel
	 * configuration.
	 * 
	 * @return the ACIDE - A Configurable IDE help menu new panel
	 * configuration.
	 */
	...
	/**
	 * Sets the ACIDE - A Configurable IDE help menu new panel configuration.
	 * @param configuration
	...
	/**
	 * Sets the ACIDE - A Configurable IDE help menu new panel 
	 * data configuration.
	 */
	public void setDataFromConfiguration(){
		// Updates the help panel configuration panel table model
		// with the data
		((AcideHelpMenuNewPanelTableModel) getTable().getModel())
				.setItems(_configuration.getItemsManager().getAllObjectsWithNumber());
	}
	
	/**
	 * Sets the listeners for the ACIDE - A Configurable IDE help panel
	...
	/**
	 * Adds the components to the ACIDE - A Configurable IDE help panel.
	 */
	...
	public void buildTable() {
		...
		_table = new JTable(new AcideHelpMenuNewPanelTableModel(
				this));
	...
	private JPopupMenu createContextMenu(final int rowIndex,
		...
		AcideMenuObjectConfiguration ob = ((AcideHelpMenuNewPanelTableModel) getTable().getModel())
	...
	/**
	 * ACIDE - A Configurable IDE help menu configuration window 
	...
	public void initComponents() {
		...
		for (int index = 0; index < _helpMenu.getItemCount(); index++) {
			...
			try {
				...
				// If the help JMenu has a menu instead of a JMenuItem
	...
	public AcideMenuObjectConfiguration getSelectedItem() {
		if (getTable().getSelectedRow() > -1){
			return ((AcideHelpMenuNewPanelTableModel) getTable().getModel())
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (name != null){
				...
				String path;
					...
					((AcideHelpMenuNewPanelTableModel) getTable().getModel()).fireTableDataChanged();
					} else {
						path = "Root/";
						submenu = menu;
						setAreThereChanges(menu.AddSubmenu(submenu, name, path));
