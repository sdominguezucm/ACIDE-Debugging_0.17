 ...
 * @see JPanel
 */
public class AcideViewMenuNewPanel extends JPanel {
	/**
	 * ACIDE - A Configurable IDE view menu panel serial version UID.
	 */
	private static final long serialVersionUID = 1L;
	/**
	 * ACIDE - A Configurable IDE view menu panel configuration.
	 */
	private AcideMenuSubmenuConfiguration _configuration;
	/**
	 * ACIDE - A Configurable IDE view new configuration window table of tool
	 * bar commands.
	 */
	...
	/**
	 * ACIDE - A Configurable IDE view menu panel file menu instance.
	 */
	...
	/**
	 * ACIDE - A Configurable IDE  view menu panel are there changes.
	 */
	private static boolean _areThereChanges;
	/**
	 * ACIDE - A Configurable IDE view menu panel selected row.
	 */
	...
	/**
	 * Sets the ACIDE - A Configurable IDE view menu new panel configuration.
	 * @param configuration
	...
	/**
	 * Sets the ACIDE - A Configurable IDE view menu new panel 
	 * data configuration.
	 */
	public void setDataFromConfiguration(){
		// Updates the file panel configuration panel table model
		// with the data
		((AcideViewMenuNewPanelTableModel) getTable().getModel())
				.setItems(_configuration.getItemsManager().getAllObjectsWithNumber());
	}
	
	/**
	 * Sets the listeners for the ACIDE - A Configurable IDE view panel
	...
	/**
	 * Adds the components to the ACIDE - A Configurable IDE view panel.
	 */
	...
	public void buildTable() {
		...
		_table = new JTable(new AcideViewMenuNewPanelTableModel(
				this));
	...
	private JPopupMenu createContextMenu(final int rowIndex,
		...
		AcideMenuObjectConfiguration ob = ((AcideViewMenuNewPanelTableModel) getTable().getModel())
	...
	public AcideMenuObjectConfiguration getSelectedItem() {
		if (getTable().getSelectedRow() > -1){
			return ((AcideViewMenuNewPanelTableModel) getTable().getModel())
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (name != null){
				...
				String path;
					...
					((AcideViewMenuNewPanelTableModel) getTable().getModel()).fireTableDataChanged();
					} else {
						path = "Root/";
						submenu = menu;
						setAreThereChanges(menu.AddSubmenu(submenu, name, path));
