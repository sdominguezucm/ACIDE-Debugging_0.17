...
public class AcideAddConsolePanelToolBarButtonWindow extends JFrame {
	...
	class AcceptButtonAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (!name.matches("") && !action.matches("")) {
				...
				// Adds the command to the console panel in the tool bar
				// configuration window
				_toolBarConfigurationWindow.getConsolePanel().addCommand(
						editableToolBarCommand);
	...
	/**
	 * ACIDE - A Configurable IDE add console tool bar button window escape key
	 * action.
	 * 
	 * @version 0.11
	 * @see AbstractAction
