...
import acide.gui.menuBar.configurationMenu.lexiconMenu.gui.defaultLexiconsWindow.utils.AcidePredetermineLexiconWindowTableModel;
...
public class AcideDefaultLexiconsWindow extends JFrame {
	...
	private void updateOpenedFileEditorsAndConsolePanel() {
		...
		for (int index = 0; index < AcideMainWindow.getInstance()
			...
			String lexiconConfiguration = AcideWorkbenchConfiguration
					.getInstance()
					.getLexiconAssignerConfiguration()
					.getLexiconConfiguration(fileEditorPanel.getFileExtension());

			// If it does not have lexicon configuration
			if (lexiconConfiguration != null)
				...
				// Loads the default lexicon configuration
				fileEditorPanel.getLexiconConfiguration().load(
						AcideLexiconConfiguration.DEFAULT_PATH
		...
		if (selectedFileEditorPanelIndex != -1) {
			...
			// Updates the components related to the selected file editor panel
			AcideMainWindow.getInstance().getFileEditorManager()
					.updateRelatedComponentsAt(selectedFileEditorPanelIndex);
		}

		// If the changes has to been applied to the console panel
		if (_applyLexiconToConsoleCheckBox.isSelected()) {

			// Checks if the console configuration exists
			File file = new File(_consoleLexiconTextField.getText());
			...
			if (file.exists()) {
				...
				// Updates the console lexicon configuration in the lexicon
				// assigner configuration with the default lexicon
				AcideWorkbenchConfiguration
						.getInstance()
						.getLexiconAssignerConfiguration()
						.setConsoleLexiconConfiguration(
			...
			// Updates the console lexicon configuration in the lexicon assigner
			// configuration with the default lexicon
			AcideWorkbenchConfiguration
					.getInstance()
					.getLexiconAssignerConfiguration()
					.setConsoleLexiconConfiguration(
	...
	/**
	 * ACIDE - A Configurable IDE default lexicons window escape key action.
	 * 
	 * @version 0.11
	 * @see AbstractAction
	 */
