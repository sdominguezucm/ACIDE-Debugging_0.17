 ...
 */
package acide.gui.menuBar.configurationMenu.lexiconMenu.gui.configurationWindow.panels.reserverdWords;
...
import acide.gui.menuBar.configurationMenu.lexiconMenu.gui.configurationWindow.utils.AcideCellColorEditor;
import acide.gui.menuBar.configurationMenu.lexiconMenu.gui.configurationWindow.utils.AcideColorCellRenderer;
...
public class AcideReservedWordsPanel extends JPanel {
	...
	private void buildComponents() {
		...
		// Sets the apply for all delimiters button vertical text position as
		// center
		_applyForAllDelimitersButton
				.setVerticalTextPosition(AbstractButton.CENTER);

		// Sets the apply for all delimiters button horizontal text position as
		// leading
		_applyForAllDelimitersButton
	...
	public JScrollPane buildTable() {
		...
		// Sets the table renderer for the color column
		_table.setDefaultRenderer(Color.class, new AcideColorCellRenderer(true));

		// Sets the table editor for the color column
	...
	public void updateFieldsWithRowSelectedInTable() {
		...
		if (selectedRow != -1) {
			...
			AcideLexiconTokenGroup tokenType = AcideMainWindow.getInstance()
	...
	public void createTableData() {
		...
		AcideLexiconTokenManager tokenTypeManager = AcideMainWindow
		...
		for (int index1 = 0; index1 < tokenTypeManager.getSize(); index1++) {

			for (int index2 = 0; index2 < tokenTypeManager.getTokenGroupAt(
					index1).getSize(); index2++) {

				// Sets the name
				_tableData[currentIndex][0] = tokenTypeManager.getTokenGroupAt(
						index1).getTokenAt(index2);

				// Sets the preview text style
				_tableData[currentIndex][1] = getPreviewText(tokenTypeManager,
						index1, index2);

				// Sets the color
				_tableData[currentIndex][2] = tokenTypeManager.getTokenGroupAt(
						index1).getColor();

				// Looks for the next index
				currentIndex++;
			}
		}
	}
	...
	/**
	 * Builds the preview text for the table in the ACIDE - A Configurable IDE
	 * reserved words panel.
	 * 
	 * @param tokenTypeManager token type manager.
	 * @param index1 index for the token group.
	 * @param index2 index for the token list in the token group at index1.
	 * 
	 * @return the preview text for the table in the ACIDE - A Configurable IDE
	 * reserved words panel.
	 */
	private String getPreviewText(AcideLexiconTokenManager tokenTypeManager,
			int index1, int index2) {

		// Builds the color for HTML format
		String color = "";

		// Gets the red component from the checked token group
		int colorInt = tokenTypeManager.getTokenGroupAt(index1).getColor().getRed();

		if (Integer.toHexString(colorInt).length() == 1) {
			color += "0" + Integer.toHexString(colorInt);
		} else {
			color += Integer.toHexString(colorInt);
		}

		// Gets the green component from the checked token group
		colorInt = tokenTypeManager.getTokenGroupAt(index1).getColor().getGreen();
		if (Integer.toHexString(colorInt).length() == 1) {
			color += "0" + Integer.toHexString(colorInt);
		} else {
			color += Integer.toHexString(colorInt);
		...
		// Gets the blue component from the checked token group
		colorInt = tokenTypeManager.getTokenGroupAt(index1).getColor().getBlue();
		...
		if (!tokenTypeManager.getTokenGroupAt(index1).getTokenAt(index2)
				.equalsIgnoreCase("<")) {
			switch (tokenTypeManager.getTokenGroupAt(index1).getFontStyle()) {

			case Font.PLAIN:
				value = "<html><head></head><body><div style=\"color:" + color
						+ ";\"><CENTER>"
						+ tokenTypeManager.getTokenGroupAt(index1).getTokenAt(index2)
	...
	class QuitButtonAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (selectedRow != -1) {
				...
				// Removes the token from the token list in the token group list
				AcideMainWindow.getInstance().getFileEditorManager()
						.getSelectedFileEditorPanel().getLexiconConfiguration()
	...
	class ApplyForAllDelimitersButtonAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			for (int index = 0; index < AcideMainWindow.getInstance()
				...
				newTokenType.setFontStyle(_previewTextField.getFont()
						.getStyle());
				newTokenType.insertToken(value);
				newTokenType.setName();
				newTokenType
				.setCaseSensitive(_caseSensitiveCheckBox.getState());
				...
				AcideMainWindow.getInstance().getFileEditorManager()
						.getSelectedFileEditorPanel().getLexiconConfiguration()
						.getTokenTypeManager().insertToken(newTokenType, value);
			}
	...
	class ModifyButtonAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (selectedRow != -1) {
				...
				// Creates the new token group to replace the old one
				AcideLexiconTokenGroup tokenGroup = new AcideLexiconTokenGroup();
				tokenGroup.setColor(_previewTextField.getForeground());
				tokenGroup.setFontStyle(_previewTextField.getFont().getStyle());
				tokenGroup.setCaseSensitive(_caseSensitiveCheckBox.getState());
				tokenGroup.insertToken(value);
				tokenGroup.setName();
