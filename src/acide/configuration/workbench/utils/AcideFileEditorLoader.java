		...
		for (int index = 0; index < AcideProjectConfiguration.getInstance()
			...
			if (projectFile.isOpened()) {
				...
				if (!file.isDirectory() && file.exists()) {
					...
					// Updates the grammar message status bar
					AcideMainWindow
							.getInstance()
							.getStatusBar()
							.setGrammarMessage(
									AcideLanguageManager.getInstance()
											.getLabels().getString("s449")
											+ " "
											+ currentGrammarConfiguration.getName());
		...
		for (int index = 0; index < fileEditorConfiguration
			...
			if (!file.isDirectory() && file.exists()) {
				...
				if (AcideProjectConfiguration.getInstance().getFileAt(
					...
					// Gets the name
					int lastIndexOfSlash = filePath.lastIndexOf(File.separator);
					...
					try {
						...
						// Throws the BadLocationException

						// Sets the caret position by default at the first
						// position
						fileEditorPanel.getActiveTextEditionArea()
								.setCaretPosition(0);
					}
