...
public class DesDatabaseManager extends AcideDatabaseManager{
	...
	public LinkedList<String> getFields(String database, String table) {
		...
		while (!found && i<columns.size()){
			...
			if (table.contains("(")) nameTable = table.substring(0,table.indexOf("("));
			...
			if (columns.get(i).startsWith(nameTable)){
				...
				if (index > 0)
					...
					info = columns.get(i).substring(columns.get(i).indexOf("(")+1, columns.get(i).lastIndexOf(")"));
	...
	public String getView(String database, String oldView) {
		...
		if (found){
			...
			for(int j=0;j<cleanViews.length;j++){
				if (j==(cleanViews.length-1)) childClear = childClear + cleanViews[j].substring(0, cleanViews[j].indexOf(":"))+")";
	...
	public LinkedList<String> getFuncionDep(String database, String table) {
		...
		while (!found && i<cons.size()){
			...
			if (cons.get(i).contains("    - ") && !cons.get(i).contains("    - FD"))
	...
	public LinkedList<String> getIntConst(String database, String table) {
		...
		while (!found && i<cons.size()){
			...
			if (cons.get(i).contains("    - ") && !cons.get(i).contains("    - IC"))
				...
				if (cons.get(i).startsWith("    - IC"))
					...
					while(i<cons.size() && !cons.get(i).startsWith(":-") && !cons.get(i).startsWith("    - IC")){
	...
	public LinkedList<String> getCandidateKey(String database, String table) {
		...
		while (!found && i<cons.size()){
			...
			if (cons.get(i).contains("    - ") && !cons.get(i).contains("    - CK"))
	...
	public LinkedList<String> getForeignKey(String database, String table) {
		...
		while (!found && i<cons.size()){
			...
			if (cons.get(i).contains("    - ") && !cons.get(i).contains("    - FK"))
	...
	public LinkedList<String> getPrimKey(String database, String table) {
		...
		while (!found && i<cons.size()){
			...
			if (cons.get(i).startsWith("    - PK")) found = true;
		...
		while (!found && i<cons.size()){
			...
			if (cons.get(i).contains("    - ") && !cons.get(i).contains("    - PK"))
				
				found = true;
			
			else{
				
				if (cons.get(i).startsWith("    - PK")) c = cons.get(i).replaceAll("    - PK:", "");
	...
	public LinkedList<String> getNullables(String database, String table) {
		...
		while (!found && i<cons.size()){
			...
			if (cons.get(i).contains("    - ") && !cons.get(i).contains("    - NN"))
	...
	public synchronized LinkedList<String> executeCommandfinal(String commandParam){
		
		LinkedList<String> res = new LinkedList<String>();
				
		AcideConsolePanel panel = AcideMainWindow.getInstance().getConsolePanel();

		panel.getProcessThread().getOutputGobbler().set_sendToConsole(false);

		panel.sendCommandToConsole(commandParam, "");

		boolean received = panel.getProcessThread().getOutputGobbler().waitForOKResult(2200);

		if(received){
						
			String[] lines = panel.getProcessThread().getOutputGobbler().getText().split("\n");

			for (int i = 0; i<lines.length ;i++ ){
				
				//dates to catch for ddbb
				if (!lines[i].startsWith("*") && !lines[i].equalsIgnoreCase("$eot") 
						&& !lines[i].startsWith("DES>") && !lines[i].startsWith(">") 
						&& !lines[i].startsWith("DES:$des>") && !lines[i].startsWith("?-")
						&& /*!lines[i].equals("$error") &&*/ !lines[i].equals("$success")
						&& !lines[i].contains("already in use.") && !lines[i].endsWith("$eot")){
	...
	public LinkedList<String> getIntConst(String database) {
		...
		if (i == 4 ){
			...
			intConst = intConst.substring(intConst.indexOf(":")+1, intConst.length());
	...
	public String test(){
		...
		if (shellPath != null
				&& (shellPath.endsWith("\\des.exe") || shellPath
						.endsWith("/des"))) {
			lines = executeCommand("/tapi /test_tapi");
		}
	...
	public boolean existsRelation(String database, String table){
		...
		if (result.get(0).contains("true")) return true;
	...
	public String insertValues(String table,Vector<String> dataColumns) {
		...
		LinkedList<String> ret = executeCommand("/tapi insert into ["+table+"] values("+data+")");
		
		executeCommand("/tapi /use_db "+ db);
		
		if(ret.size()==1) result= "success";
		
		else result = ret.get(2);
	...
	public String dropView(String database, String view){
		...
		while (i<result.size() && !error){
			...
			if (result.get(i).contains("$error") || result.get(i).contains("Error"))
	...
	public String pasteTable(String destino, String origen, int option){
		...
		if (option == 1){
			...
			command ="/tapi insert into "+destino+" select * from "+origen;
		...
		if(res.isEmpty() || res.size()==1)return "success";
		
		else return (String) res.get(2);
	}
	...
	public boolean isEmpty(String db, String table) {
		...
		if(res.get(0).contains("true"))return true;
	 ...
	 * @return String
	 */
	public String deleteRow(String database, String table,Vector<Vector<String>> dataColumns,Vector<String> columnNames) {
		...
		for(int row=0; row<dataColumns.size(); row++){
			...
			int recordCount = getRecordCount(database,table,columnNames,dataColumns.get(row));
			...
			if(!listpk.isEmpty()){ //tiene pk
				...
				for(int i=0; i<g.length; i++){
					...
					command += ((String) g[i]).trim()+"]="+dataColumns.get(i);
				...
				res = deleteRows(database,table,dataColumns.get(row), columnNames); //borramos

				if (!res.equals("success")) return res;
				
				if(recordCount>1){// hay repetidos y no tiene pk
				
					for(int i=0; i<recordCount-1; i++){ //insertamos num-1 tuplas
	...
	public String deleteRows(String database, String table,Vector<String> dataColumns,Vector<String> columnNames) {
	...
	public String updateField(String table, String database,Vector<String> columnsNames, Vector infoRow, Vector newInfoRow) {
		...
		if(!listpk.isEmpty()){ //tiene pk
			...
			for(int i =1; i <columnsNames.size();i++){
				...
				command = command +" , "+ columnsNames.get(i)+"="+newInfoRow.get(i)+" ";
			...
			if(recordCount>1){// hay repetidos y no tiene pk
				...
				res = insertValues(table, newInfoRow); //insertamos el modificado
				...
				for(int i =1; i <columnsNames.size();i++){
					...
					command += " , ["+ columnsNames.get(i)+"]="+newInfoRow.get(i)+" ";
				}
				
				command = command+"WHERE ["+ columnsNames.get(0)+"]="+infoRow.get(0)+" ";
				...
				for(int i =1; i <columnsNames.size();i++){
					...
					if(data==null || data.equals(""))
						...
						command += " and ["+columnsNames.get(i)+"]="+infoRow.get(i)+" ";
				
				}

				executeResult = executeCommand(command);

				if(((String) executeResult.get(0)).contains("error")) res =(String) executeResult.get(2);
	...
	private int getRecordCount(String database, String table,Vector columnsNames, Vector infoRow) {
		...
		String command ="/tapi SELECT COUNT(*) FROM "+table+" WHERE "+columnsNames.get(0)+"="+infoRow.get(0)+" ";
		...
		try{
			...
			result=Integer.valueOf((String) executeResult.get(executeResult.size()-1));
	...
	public LinkedList<String> orderColumns(String table, String command){
		...
		String commandLine = "/tapi SELECT * FROM " + table + " ORDER BY " + command;
